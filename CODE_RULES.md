...
## 6. Display Domain Values Through Label Mappers

- Never render a stored or wire value directly when it is an enum-like code such as `owner`, `member`, `lead`, `workspace`, `private`, `ok`, or `error`. Users must see human-readable copy, not the code.
- Convert a value to a label with an explicit, typed function (for example `getOrganizationRoleLabel`) backed by a typed label record. Do not mechanically capitalize or otherwise transform a code at the render site.
- Keep one label mapping per domain in `packages/shared` so both apps agree, and make it exhaustive over the domain union so adding a value fails type checking until its label exists.
- For Base UI `Select`, pass the label record through the root `items` prop so `SelectValue` renders the label instead of the raw selected value, and use the same mapper for `SelectItem` children.
- This applies everywhere a coded value can reach the screen: selects and their triggers, badges, tables, cards, headings, dialogs, tooltips, toasts, and accessible names such as `aria-label` and `title`.
- Free-form display data such as names, emails, descriptions, counts, and timestamps formatted with `formatDate` is already display-ready and needs no label mapper.

## 7. Respect the shadcn Design System
...
- When rendering a form, add `my-4` to its `FieldGroup` (or the form element) so the form is separated from the surrounding components.

## 8. Use One Naming Convention
